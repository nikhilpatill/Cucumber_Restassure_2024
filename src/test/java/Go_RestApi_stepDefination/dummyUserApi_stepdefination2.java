...
public class dummyUserApi_stepdefination2 {
	...
	public void create_dummyuser_user_payload() {
		...
		LoginData.mapLogin_User("kminchelle", "0lelplR");
	...
	public void login_token_dummyuser_user_api_using_post_method() {
		...
		res = given().log().all().header("Authorization",
				...
				.body(LoginData.mapLogin_User("kminchelle", "0lelplR")).when().post().then().log().all()
